...
class Info(BaseModel):
    station: Literal['W 21 St & 6 Ave',
                     'University Pl & E 14 St', '8 Ave & W 31 St']
...
def predict_day(model, info):
    try:
        ...
        df_history = pd.read_csv("2025_timeseries.csv", index_col=0, header=[
                                 0, 1], parse_dates=True)
    df_history = df_history.stack(
        level=[0, 1], future_stack=True).reset_index()
    df_history.columns = ['time', 'station', 'rideable_type', 'stock']

    df_history['station'] = df_history['station'].astype('category')
    df_history['rideable_type'] = df_history['rideable_type'].astype(
        'category')
    ...
    data['date'] = (data['time'].dt.normalize().astype(
        np.int64) - start_ts) / (end_ts - start_ts)

    target_mask = (data['time'] >= pd.to_datetime(info.target_date + " 00:00:00")) & \
                  (data['time'] <= pd.to_datetime(
                      info.target_date + " 23:45:00"))
